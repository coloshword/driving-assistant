...
- (void)loadModel:(NSString *)modelDir
  ...
  dispatch_async(_ttsQueue, ^{
    ...
    for (NSString *f in @[@"model.onnx", @"voices.bin", @"tokens.txt", @"espeak-ng-data"]) {
  ...
  dispatch_async(_ttsQueue, ^{
    ...
    const SherpaOnnxGeneratedAudio *audio =
      SherpaOnnxOfflineTtsGenerateWithCallbackWithArg(
        self->_tts, text.UTF8String, (int32_t)speakerId, (float)speed, KokoroStreamCallbackWithArg, &ctx);
    ...
    double audioMs = audio->sample_rate > 0 ? (1000.0 * (double)audio->n / (double)audio->sample_rate) : 0.0;
    NSLog(@"[DAKokoro][Benchmark] generate_ms=%.0f audio_ms=%.0f rtf=%.3f chunks=%d",
          generateMs, audioMs, audioMs > 0 ? generateMs / audioMs : -1, ctx.chunkCount);
